...
Future<SelectedMedia?> selectMedia({
  ...
  log('Heree');
  final path = await storagePath(FirebaseAuth.instance.currentUser!.uid,
      pickedMedia!.name, isVideo, isUserImage, isKYC);
  ...
  ScaffoldMessenger.of(context)
    ...
    ..showSnackBar(SnackBar(
      content: MessageWidget(
        message: 'Invalid file format: ${mime(filePath)}',
        isError: true,
      ),
    ));
  return false;
...
Future<String> storagePath(String uid, String filePath, bool isVideo,
    bool isProfile, bool isPan) async {
  final timestamp = (DateTime.now()).microsecondsSinceEpoch;
  // Workaround fixed by https://github.com/flutter/plugins/pull/3685
  // (not yet in stable).
  final ext = isVideo ? 'mp4' : filePath.split('.').last;
  if (isProfile)
    return 'profile/$uid.$ext';
  else if (isPan)
    return 'kycPan/$uid.$ext';
  else
    return 'users/$uid/thumbnails/$timestamp.$ext';
}
